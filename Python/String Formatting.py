...
def print_formatted(number):
    lengths = [len(str(number)), len(oct(number)) - 2, len(hex(number)) - 2, len(bin(number)) - 2]
    
    for i in range(1, number + 1):
        l = [str(i).rjust(lengths[3]), str(oct(i)[2:]).rjust(lengths[3]), str(hex(i)[2:].upper()).rjust(lengths[3]), str(bin(i)[2:]).rjust(lengths[3])]
        line = " ".join(l)
