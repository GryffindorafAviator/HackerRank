...
# The maximum value that you can obtain, 
# will be the answer to the problem.

# Input Format
# The first line contains 2 space separated integers K and M.
# The next K lines each contains an integer Ni, 
# denoting the number of elements in the ith list, 
...
# Explanation
# Picking 5 from the 1st list,
# 9 from the 2nd list and 10 from the 3rd list gives the maximum S value equal to (5^2 + 9^2 + 10^2) % 1000 = 206.

# Solution
from itertools import product

k, m = map(int, input().split())
numbers = [[int(num) ** 2 for num in input().split()[1:]] for _ in range(k)]
print(max(sum(i) % m for i in product(*numbers)))
