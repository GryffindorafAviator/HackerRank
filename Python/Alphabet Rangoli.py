...
# Only one line of input containing size, 
# the size of the rangoli.
...
def print_rangoli(size):
    for i in range(2 * size - 1):
        ...
        lett = chr(ord('a') + offset)
        l = inline_pattern(size, lett, offset)
        print(l)
        
def inline_pattern(size, lett, offset):
    if size == 1:
        return 'a'
